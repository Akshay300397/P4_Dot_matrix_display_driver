...
// ─────────────────────────────────────────────────────────────
//  RESPONSE STRINGS
//  Sent back over UART → WIZ550S2E → TCP to the sender
//
//  Success:  "OK:D1\r\n" through "OK:D6\r\n"  (field confirmed)
//  Errors:   "CRC_ERR\r\n"  — CRC mismatch
//            "LEN_ERR\r\n"  — LEN exceeds max
//            "C/D_ERR\r\n"  — unknown C/D identifier
// ─────────────────────────────────────────────────────────────
 
#define RESP_CRC_ERR    "CRC_ERR\r\n"
#define RESP_LEN_ERR    "LEN_ERR\r\n"
#define RESP_CD_ERR     "C/D_ERR\r\n"

// Helper — sends a null-terminated string out over UART (excludes null)
static inline void uart_send(const char *str)
{
...
static void parse_frame(const uint8_t *frame)
{
    ...
    if (len > FRAME_PAYLOAD_LEN) {
        ESP_LOGW(TAG1, "LEN_ERR: len=%d max=%d", len, FRAME_PAYLOAD_LEN);
        uart_send(RESP_LEN_ERR);
        return;
    }
    ...
    if (calc != rx_crc) {
        ESP_LOGW(TAG1, "CRC_ERR: calc=0x%04X rx=0x%04X", calc, rx_crc);
        uart_send(RESP_CRC_ERR);
        return;
    }
    ...
    if      (strcmp(cd, MODE_DATA_F1) == 0) field = 1;
        ...
        ESP_LOGW(TAG1, "C/D_ERR: unknown [%s]", cd);
        uart_send(RESP_CD_ERR);
        return;
    }
    ...
    display_update_from_uart(field, text);
     ...
     // ── ACK: confirm field update back to sender ──────────────
    // Format: "OK:D1\r\n" ... "OK:D6\r\n"
    char ack[10];
    snprintf(ack, sizeof(ack), "OK:%s\r\n", cd);   // cd is already "D1".."D6"
    uart_send(ack);
}
